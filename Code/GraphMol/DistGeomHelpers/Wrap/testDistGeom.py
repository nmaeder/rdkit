...
class TestCase(unittest.TestCase):
    ...
    def test1Small(self):
        ...
        rdDistGeom.EmbedMolecule(
            mol, 10, 1, useExpTorsionAnglePrefs=False, useBasicKnowledge=False
        )
        conf = mol.GetConformer()
    ...
    def test4OrderDependence(self):
        self.assertTrue(
            compareOrder(
                "CC(C)(C)C(=O)NC(C1)CC(N2C)CCC12", "CN1C2CCC1CC(NC(=O)C(C)(C)C)C2"
            )
        )
        # issue 230
        self.assertTrue(
            compareOrder("C#CC(C)(C)N(CN1)C\\N=C/1SC", "CSC1=NCN(C(C)(C)C#C)CN1")
        )
        # issue 232
        self.assertTrue(
            compareOrder(
    ...
    def test5Issue285(self):
        ...
        for i, ci in enumerate(cs):
            for j in range(i + 1, len(cs)):
                cj = cs[j]
                self.assertTrue(
                    Chem.MolToMolBlock(m, confId=ci) != Chem.MolToMolBlock(m, confId=cj)
                )
    ...
    def test8MultiThreadMultiConf(self):
        ...
        for cid in cids:
            ...
            for i in range(mol.GetNumAtoms()):
                msd += (
                    mol.GetConformer().GetAtomPosition(i)
                    - mol2.GetConformer().GetAtomPosition(i)
                ).LengthSq()
            msd /= mol.GetNumAtoms()
    ...
    def testETKDGIsDeterministic(self):
        ...
        self.assertDeterministicWithSeed(
            195225787
        )  # one higher seed will overflow though
    ...
    def testProvidingCPCI(self):
        ...
        for i in range(5):
            ...
            self.assertTrue(
                (conf2.GetAtomPosition(3) - conf2.GetAtomPosition(0)).Length()
    ...
    def testScaleBoundsMatForce(self):
        ...
        for i in range(5):
            ...
            self.assertTrue(
                abs(
                    (conf2.GetAtomPosition(4) - conf2.GetAtomPosition(0)).Length()
                    - target
                )
                < abs(
                    (conf1.GetAtomPosition(4) - conf1.GetAtomPosition(0)).Length()
                    - target
                )
            )
    ...
    def testETKDGv3amide(self):
        ...
        for torsion in get_atom_mapping(mol):
            ...
            self.assertAlmostEqual(
                abs(ComputeSignedDihedralAngle(a1, a2, a3, a4)), 3.14, delta=0.1
            )
